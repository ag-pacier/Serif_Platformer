...
@onready var menu_items: Array = [$VBoxContainer/LastItem]
...
func _ready() -> void:
	...
	$VBoxContainer/MenuTitle.text = menu_title
	if top_menu:
		$VBoxContainer/LastItem.text = "Exit"
	else:
		$VBoxContainer/LastItem.text = "Back"
...
func rebuild_menu_items(new_items: Array) -> void:
	new_items.append_array([$VBoxContainer/LastItem])
